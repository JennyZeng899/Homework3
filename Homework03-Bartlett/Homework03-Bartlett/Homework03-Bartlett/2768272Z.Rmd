...
All the model and data that I use are avaliable at https://github.com/JennyZeng899/Homework3.git. Feel free to check and reproduce. 
...
Ghaderi-Kangavari, A., Parand, K., Ebrahimpour, R., Nunez, M. D., & Rad, J. A. [-@ghaderi2021spatial]. How spatial attention affects the decision process: Looking through the lens of bayesian hierarchical diffusion model & EEG analysis. bioRxiv, 2021–2005.
...
The process of spatial prioritization in perceptual decision-making can not be observed directly. It is modeled by a well-established sequential sampling model, known as the drift-diffusion model (DDM) which involved different parameters: the drift rate, the boundary separation, and the non-decision time.

In order to find the best fit model for spatial prioritization, researcher first used a hierarchical Bayesian estimation approach to explore the latent cognitive processes underlying perceptual decisions during spatial prioritization, and to link those latent cognitive processes to neural mechanisms. 
...
Researcher conduct five different models and compared the performance of these five models to see which model best fit the data. 
...
While the researchers provided an explanation for using Bayesian statistics in the cognitive model, they did not offer a justification for why using it in multiple regression model.
...
+ cognitive modelling: Researcher clearly stated that *a Bayesian hierarchical approach is considered, which allows inferences to be performed simultaneously on the group and individual level.* in the abstract explain why they intersted in using Bayesian approach. They also clearly stated the aim of the analysis is to explore the latent cognitive processes underlying perceptual decisions during spatial prioritization, and to link those latent cognitive processes to neural mechanisms. This provide reader an better insight into why researcher aiming to use Bayesian approach. 

+ multiple regression modeling: The researcher's explanation for the use of Bayesian tests in their explanation was unclear and lacking in detail. The calculation of the Bayes factor (BF) appeared sudden and confusing.

#### Step 1. Explain the model

in this part of evaluation we need to check the data variables, likelihood function and parameter, prior distribution, formal specification and prior predictive check. 

+ cognitive modelling: this research paper only specific the formal (X(t+∆t) = X(t) +v·∆t +e·√∆t,) they aim to use and no other information provide. Furthermore, the formula is located in the introduction, which can be confusing for readers as it may appear to be the model for the hypothesis. Additionally, the experimental procedure has not been mentioned yet, which makes this part even more difficult to understand.

Although the researcher does spend time explaining what each parameter means in the equation, the explanation may not make sense until the reader finishes reading the behavioral experiment design section. Therefore, the researcher should have considered the structure of the article more carefully and how the information may be presented in a more understandable way for readers.

+ multiple regression modeling: The model equation y = β0 + β1X1 + β2X2 + ε is helpful for understanding the modeling process, but the absence of information about the prior and likelihood function can leave readers wondering. Providing more context on the prior distribution and likelihood function used in the model would help readers better understand the modeling process and the basis for the posterior distribution. It would be beneficial if the researcher provided a clear explanation of the Bayesian approach they used, including the prior distribution and likelihood function, to help readers understand how the model was constructed and the assumptions underlying it.

#### step 2. report details of the computation

+ cognitive modelling: The researcher made a commendable effort in describing the modeling process by mentioning the use of the HDDM package in Python. They were able to generate the full posterior distributions of the model parameters at both the individual and group level by employing Markov-chain Monte Carlo sampling (MCMC). The sampling progress can be observed from the converged chains provided for each model. However, it would have been more informative if the researcher had reported the effective sample size (ESS) of the chains to allow readers to assess the resolution of the chains.

+ multiple regression modeling: The researchers introduced their use of the BayesFactor package in R and calculated BF10, which was reported with a clear subscript. But they haven't clearly state the statistical hypothesis they interest to test.

#### step 3. Describe the posterior distribution

+ cognitive modelling: This paper includes a posterior predictive check which compares the posterior distribution to the original dataset. The authors also plot the posterior distribution and include the 95% highest posterior density (HPD). While this is a valuable contribution to the analysis, the authors do not provide a summary of their findings or any accompanying text to interpret the results. Adding a brief interpretation of the results would greatly enhance the reader's understanding of the paper.

+ multiple regression modeling: While the researchers reported a clear Bayes Factor (BF) with a subscript, they did not provide posterior probabilities of models for a range of prior model probabilities. As the prior was not mentioned in the beginning, this information would be helpful in understanding the robustness of their findings to different prior assumptions.

#### STEP 4. Report decision and their criteria

+ cognitive modelling: The researchers presented a clear table comparing the DIC and R square values for each model, which is a crucial step in deciding which model performs better. They also provided instructions on how to interpret the results, stating that a lower DIC score is preferred, while an R square value closer to one indicates a better fit. 

+ multiple regression modeling: The researcher also provided a clear instruction on how to decide which hypothesis is better supported based on the Bayes Factor. However, they did not state or justify the decision threshold for the posterior model probability for both models. Additionally, there was no information on the minimum prior model probability that would make the posterior model probability exceed the decision threshold. These details are important for readers to understand how the researcher reached their conclusions and how confident they are in their findings.

#### Step 5. Report sensitivity analysis

We will discuss both models together in the last two steps since the issue is the same for both models - no information about the prior at all. Therefore, there is no way to check the model sensitivity to different priors. The paper also lacks posterior model probabilities for a range of prior model probabilities for both model comparison and hypothesis testing. This information is important for understanding the robustness of the results and the degree of support for the hypotheses. Without this information, the conclusions of the study may be less convincing, and it is difficult to determine the practical significance of the findings. It is essential for researchers to report the prior information and sensitivity analyses to provide readers with a more comprehensive understanding of the results.

#### Step 6. Make it reproducible

The researchers in this paper provided information about the software and packages used and also shared the code and simulation scripts for all current models (https://github.com/AGhaderi/hDDM_attention).  

However, the paper is a bit messy and hard to follow, and they did not provide version information about the software they used. While the reproducibility seems to be adequate, there are still several missing pieces of information, particularly related to Bayesian testing and estimation. Additional information about the prior could help readers better understand the paper. In conclusion, while there are some positive aspects of the paper, there is also room for improvement in terms of clarity and completeness.
...
Based on the research question "How does workplace quality, manager influence, and peer influence affect unethical workplace intentions?", the statistical hypotheses for each variable can be summarized as follows, all the hypothesis are two-tails:
...
Based on the instruction provided, it appears that the Evidence Ratio (ER) is the key value of interest for evaluating the hypothesis tests. The ER is equal to the Bayes factor BF01. The ER are `r BFh01[["hypothesis"]][["Evid.Ratio"]]`, `r BFh02[["hypothesis"]][["Evid.Ratio"]]`, `r  BFh03[["hypothesis"]][["Evid.Ratio"]]`, `r  BFh04[["hypothesis"]][["Evid.Ratio"]]`, `r  BFh05[["hypothesis"]][["Evid.Ratio"]]`, `r  BFh06[["hypothesis"]][["Evid.Ratio"]]` and `r  BFh07[["hypothesis"]][["Evid.Ratio"]]`, separately. We than can compare this value to the guideline from Wagenmakers et al. [-@wagenmakers2011psychologists]. The introduction suggests that when the evidence ratio (BF01) is below 1, it indicates support for the alternative hypothesis over the null hypothesis. 
```{r}
 knitr::include_graphics("BH01.png") 
```

Interestingly, except for the hypothesis related to the interaction between workplace quality and peer influence (BFH05 = `r BFh05[["hypothesis"]][["Evid.Ratio"]]`), the evidence ratios for all other hypotheses are below 1. This suggests that there is anecdotal evidence in favor of the alternative hypotheses, indicating that we may need to reject the null hypothesis and consider that the variables have an influence on individual unethical behavior intentions. These results seem to contradict our previous conclusions. One possible reason for this discrepancy could be that the variables have effects in both directions, and hence we cannot draw conclusions about the direction of the effect by simply comparing the coefficients to a point-value of 0 based on the 95% highest density interval (HDI).
...
```{r}
# the sampling process result for each model is really long. plz click the End of sampling on the left to directly jump to the model comparison sessions.
...
The objective of this study is to explore the relationship between workplace quality, manager influence, and peer influence on unethical workplace intentions. Jones and Kavanagh's (1996) research found that unethical behavior intentions were consistently higher in low-quality workplaces, but the impact of manager and peer influence was not consistent. They also observed that peer influence had a greater impact on unethical behavior intentions among undergraduate participants with limited work experience, whereas for older participants with more work experience, manager influence had a greater impact. Additionally, the researchers observed that the impact of peer influence was obscured by the interaction effect between peer influence and manager influence. This research has not been replicated until recently. The aim of this study is to investigate the impact of workplace quality, manager influence, and peer influence on unethical workplace intentions, and evaluate the data using Bayesian statistics. Our hypotheses are that each of these factors will have a separate effect on unethical behavior intentions. Additionally, we will include interaction terms to investigate any potential combined effects. To achieve this, we have used a multiple linear regression model to analyze the data.

our two tail statistic hypothesis are list below:
+ H1 Alternative: Poor workplace quality will have an effect on individual unethical behavioral intentions. 
+ H1 Null: Poor workplace quality will not have an effect on individual unethical behavioral intentions.
...
The study considers workplace quality, manager influence, and peer influence as independent variables, each with two levels, and individual unethical behavior intention as the dependent variable. The prior specifications for the intercept, coefficients, and sigma are based on previous findings and are justified as follows: intercept - normal distribution with a mean of 7 and standard deviation of 2.6; coefficients - normal distribution with a mean of 0 and standard deviation of 0.75; sigma - exponential distribution with a rate parameter of 1. Further details on the justification of the priors can be found in the full report and the Jones and Kavanagh's (1996) paper.
...
We use R 4.2.2 and package brms, bayestestR, tidybayes, see, emmeans, patchwork and tidyverse to help us analysis and visualized the data. Markov Chain Monte Carlo simulations (MCMC) chains were used to generate 4000 samples to generate postior distribution. 

```{r, echo=FALSE}
plot(Evans_fit1)
```

Based on the summery, The hat values are equal to 1, the ESS values are larger than 1000, and the chain visualization shows no indication of getting stuck at a single value. These findings indicate that there are no issues with the model convergence. Furthermore, the plot below indicates that the model is able to capture the data pattern.
...
However, after we conductedfurther evaluation which is hypothesis testing for each group and obtained evidence ratios of BFH01 = 0.9048768, BFH02 =  0.8773134, BFH03: 0.8430152, BFH04: 0.8340765, BFH05: 1.1024213, BFH06: 0.5698168, and BFH07: 0.934855. Based on Wagenmakers et al. (2011), when the evidence ratio is below 1, it suggests support for the alternative hypothesis over the null hypothesis. With the exception of H5, all the other ratios below one suggest that there is anecdotal evidence to support those alternative hypotheses.

We also visualized the effects of variables to examine their interaction.
...
In order to check the sensitivity of our model, we first removed all the interactions and compared the R-squared values. As shown in the table, we observed that the model with all interactions included had the highest R-squared estimate, and the R-squared estimate consistently decreased as the number of interactions involved in the model decreased. This indicates that it is worthwhile to include interactions in the model.
...
Additionally, since we used a very restrictive prior in the beginning, we examined the model with the default prior and a weaker prior. The order of the summaries presented below are for the current model, the model with the default prior, and the model with the weaker prior (intercept - normal distribution with a mean of 7 and standard deviation of 3; coefficients - normal distribution with a mean of 0 and standard deviation of 1.5; sigma - exponential distribution with a rate parameter of 0.75).
...
Upon examining the summary, it becomes apparent that in the default prior and weaker prior models, the coefficient of the interaction between workplace_quality1 and peer influence is negative, contrary to the direction in our current model. This finding suggests that the prior specifications we used may have been too restrictive. As a result, it may be necessary to revisit our prior assumptions and consider using a less stringent prior to enable a more nuanced analysis and interpretation of the results.

I had upload all the model and the data that i used to the github (link at every beginning), feel free to download and re-run it. 
...
As for the interactions, since they are of personal interest, we do not set a specific direction for the hypotheses and are interested in exploring whether there are any significant effects of each interaction on the outcome，therefore the hypothesis will be in two tails.
...
The objective of this study is to investigate the influence of intrinsic motivation, academic self-regulation, and statistics anxiety on students' passive procrastination. Dunn (2014) used GLS (self-regulation), intrinsic motivation, and STARS (statistics anxiety) as predictor variables and PASS (passive procrastination) as the outcome variable to examine their effects on passive procrastination. Dunn found that the effect of GLS on passive procrastination was significant. We aim to replicate Dunn's research with face-to-face students and explore if we can identify any new findings. Despite the assumption of independence, normality, and homoscedasticity for the variables, we will include interactions in our model to investigate any possible effects of interactions.

We have developed a set of hypotheses for our study, with the first three having a specific direction, indicating our expectation for them, and requiring a one-tailed analysis. The remaining hypothesis is of personal interest, and thus, we will test it with a two-tailed analysis.

+ H1 Alternative: Academic self-regulation will have a negative effect on passive procrastination. 
+ H1 Null: Academic self-regulation will have no effect on passive procrastination.

+ H2 Alternative: Intrinsic motivation will have a negative effect on passive procrastination. 
+ H2 Null: Intrinsic motivation will have no effect on passive procrastination.

+ H3 Alternative: Statistics anxiety will have a positive effect on passive procrastination. 
+ H3 Null: Statistics anxiety will have no effect on passive procrastination.

+ H4 Alternative: The interaction between academic self-regulation (GSL_mean) and intrinsic motivation (Intrinsic_mean) will affect passive procrastination. 
+ H4 Null: The interaction between academic self-regulation (GSL_mean) and intrinsic motivation (Intrinsic_mean) will have no effect on passive procrastination.

+ H5 Alternative: The interaction between academic self-regulation (GSL_mean) and statistics anxiety (STARS_mean) will affect passive procrastination. 
+ H5 Null: The interaction between academic self-regulation (GSL_mean) and statistics anxiety (STARS_mean) will have no effect on passive procrastination.

+ H6 Alternative: The interaction between intrinsic motivation (Intrinsic_mean) and statistics anxiety (STARS_mean) will affect passive procrastination. 
+ H6 Null: The interaction between intrinsic motivation (Intrinsic_mean) and statistics anxiety (STARS_mean) will have no effect on passive procrastination.

+ H7 Alternative: The interaction between academic self-regulation (GSL_mean), intrinsic motivation (Intrinsic_mean), and statistics anxiety (STARS_mean) will affect passive procrastination. 
+ H7 Null: The interaction between academic self-regulation (GSL_mean), intrinsic motivation (Intrinsic_mean), and statistics anxiety (STARS_mean) will have no effect on passive procrastination.

The prior distributions for the model parameters were chosen based on Dunn's (2014) findings. The intercept was assigned a normal distribution with a mean of 15 and a standard deviation of 5, while the coefficients were assigned a normal distribution with a mean of 0 and a standard deviation of 1.25. The standard deviation parameter, sigma, was assigned an exponential distribution with a rate parameter of 0.5. A full report detailing the justification for these priors is available upon request.
...
Dunn_priors   %>% 
  ...
  theme_classic()
```
e used R 4.2.2 and several packages including brms, bayestestR, tidybayes, see, emmeans, patchwork, and tidyverse to analyze and visualize the data. Markov Chain Monte Carlo simulations were run with 4000 samples to generate posterior distributions. 
```{r}
plot(Dunn_fit1)
```

The hat values were all equal to 1, the ESS values were larger than 1000, and the chain visualization showed no indication of getting stuck at a single value, indicating that the model converged well. Furthermore, the plot below demonstrates that the model was able to capture the data pattern.
...
Based on the HDI plot, it can be seen that the Highest Density Intervals (HDI) for the variables GLS and intrinsic motivation do not include zero, suggesting that GLS has a negative effect and intrinsic motivation has a positive effect on passive procrastination. This is opposite to our initial expectations, which predicted a negative effect for intrinsic motivation. To gain a deeper understanding of the data, we conducted hypothesis tests. For H1 (GLS), the evidence ratio was 0, providing extreme evidence in favor of the hypothesis that self-regulation has a negative effect on passive procrastination. For H2, the evidence ratio was 77.4313725, indicating very strong evidence in favor of the hypothesis that intrinsic motivation has a positive effect on passive procrastination. For H3, although the plot did not show a significant effect, there was anecdotal evidence (ER = 0.4020329) supporting the hypothesis that statistics anxiety has a positive effect on passive procrastination. However, the evidence ratios for H4, H5, H6, and H7, which were hypotheses about the interactions, were all greater than 1, suggesting that the evidence supports H0 (no interaction), consistent with previous research. Therefore, we will not focus on interaction effects in our subsequent analyses.

As we observed a clear interaction among different variables, we visualized the changes in the plot.
...
However, after examining the plot, we found that although there was anecdotal evidence to support H3, the influence appeared to be nearly flat. 

In addition, we investigated the sensitivity of the model to the removal of interactions and the use of different priors. The table below shows the R-square estimate with and without interactions.
...
Interestingly, despite our previous finding suggesting that interactions have no effect on passive procrastination, we observed that the model with interactions included had a slightly higher R-square than the model without interactions, although the difference was not substantial. 

We then focused on how the model was sensitive to the prior. The summary order was for the current prior model and the default prior model. By comparing the short posterior summary, we observed that the prior we chose had very little influence on the final result. This suggests that our results are robust to these two choices of prior, and my prior choice is not too restrictive for this question.
...
Finally, I have uploaded all the models and data that I used to GitHub (link provided at the beginning). Please feel free to download and re-run them.
